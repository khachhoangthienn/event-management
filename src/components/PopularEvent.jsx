...
const PopularEvent = () => {
    ...
    return (
        <div className='flex flex-col container items-center gap-4 justify-start mx-auto py-10 px-6 md:px-20 lg:px-32'>
            <h1 className='text-5xl font-bold text-cyan-900'>POPULAR EVENT</h1>
            ...
            <div className='w-full grid grid-cols-auto gap-10 pt-5 gap-y-6 px-3 sm:px-0 text-gray-800'>
                {showEvents.slice(indexPage * 9, indexPage * 9 + 9).map((item, index) => (
            ...
            <div className='flex justify-center items-center gap-4' id='nav-event'>
                <a href="#nav-event" onClick={() => { setindexPage(prev => prev == 0 ? prev : prev - 1); scroll(0, 0) }} className='bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10 hover:bg-cyan-900 hover:text-white'>Previous</a>
                ...
                {indexPage * 9 + 9 < showEvents.length && <p className='text-cyan-700 text-sm mt-10'>{indexPage + 2}</p>}
                <a href="#nav-event" onClick={() => { setindexPage(prev => prev * 9 + 9 > showEvents.length ? prev : prev + 1); scrollTo(0, 0) }} className='bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10  hover:bg-cyan-900 hover:text-white'>Next</a>
            </div>
        </div>
    )
}
